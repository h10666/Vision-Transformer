...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3, num_frames=1):
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, in_channels, fusion, num_layers, vis, stochastic_droplayer_rate=0.0, if_shift=True):
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, in_channels=3, num_layers=12, fusion=False, num_classes=21843,
                 zero_head=False, vis=True, stochastic_droplayer_rate=0.0, if_shift=True):
        ...
        self.transformer = Transformer(config, img_size, in_channels, fusion, num_layers, vis,
                                       stochastic_droplayer_rate=stochastic_droplayer_rate, if_shift=if_shift)
...
class complete_model(nn.Module):
    def __init__(self, config, num_classes=21843, zero_head=True):
        ...
        self.model_visual = VisionTransformer(config, img_size=224, in_channels=3, num_layers=10, fusion=False, num_classes=num_classes,
                                              zero_head=True, stochastic_droplayer_rate=config.stochastic_droplayer_rate, if_shift=True)
    ...
    def forward(self, x, labels=None, mask=None):
        ...
        tokens_visual = self.model_visual(x['RGB'])               # B*8, 197, 768
        tokens_audio = self.model_audio(x['Spec'])                # B*8, 65, 768
        tokens_classifier = torch.add(tokens_visual[:, 0], tokens_audio[:, 0]) / 2
        tokens_classifier = torch.reshape(tokens_classifier, (B, 8, -1))
        tokens_classifier = torch.mean(tokens_classifier, dim=1)
        ...
        tokens_visual = torch.reshape(tokens_visual[:, 1:], (B, 1568, -1))
        tokens_audio = torch.reshape(tokens_audio[:, 1:], (B, 512, -1))

        # tokens_visual = tokens_visual[~mask['RGB']].reshape(B, -1, 768)
        # tokens_audio = tokens_audio[~mask['Spec']].reshape(B, -1, 768)
