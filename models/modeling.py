...
class Embeddings(nn.Module):
    ...
    """
    def __init__(self, config, img_size, imgH_size=None, in_channels=3, num_frames=8):
        super(Embeddings, self).__init__()
        self.hybrid = None
        if imgH_size is None:
            img_size = _pair(img_size)
        else:
            img_size = (imgH_size, img_size)
...
class Transformer(nn.Module):
    def __init__(self, config, img_size, vis):
        ...
        self.embeddings_visual = Embeddings(config, img_size=img_size, num_frames=1)
        self.embeddings_audio = Embeddings(config, img_size=128, in_channels=1, num_frames=1)
        self.encoder = Encoder(config, vis)

    def forward(self, input_ids):
        embedding_output_visual = self.embeddings_visual(input_ids['RGB'])
        embedding_output_audio = self.embeddings_audio(input_ids['Spec'])
        embedding_output = torch.cat((embedding_output_visual, embedding_output_audio), dim=1)
        # embedding_output = embedding_output_visual
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, labels=None):
        ...
        logits_video = self.head(embedding)                                               # B, 768 -> B, 10
        
        embedding = x[:, 197].view(x[:, 197].shape[0] // 8, 8, self.config.hidden_size)       # B*8, 768 -> B, 8, 768
        embedding = torch.mean(embedding, dim=1)                                                # B, 8, 768 -> B, 768
        logits_audio = self.head(embedding)                                                     # B, 768 -> B, 10
        logits = torch.add(logits_video, logits_audio) / 2
    ...
    def load_from(self, weights):
        with torch.no_grad():
            ...
            self.transformer.embeddings_visual.patch_embeddings.weight.copy_(np2th(weights["embedding/kernel"], conv=True))
            ...
            if self.transformer.embeddings_visual.hybrid:
                self.transformer.embeddings_visual.hybrid_model.root.conv.weight.copy_(np2th(weights["conv_root/kernel"], conv=True))
            ...
            if self.transformer.embeddings_audio.hybrid:
                self.transformer.embeddings_audio.hybrid_model.root.conv.weight.copy_(np2th(weights["conv_root/kernel"], conv=True))
                gn_weight = np2th(weights["gn_root/scale"]).view(-1)
                gn_bias = np2th(weights["gn_root/bias"]).view(-1)
